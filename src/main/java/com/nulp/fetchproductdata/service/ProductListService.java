...
public class ProductListService {
    ...
    public List<Product> getProductsByCategoryId(long categoryId) {
        ...
        List<ProductDetails> productDetailsList = StreamSupport.
                stream(Iterables.partition(productIds, bufferSize).spliterator(), false)
                .map(productDetailsParser::getProductDetailsByProductId).flatMap(List::stream)
                .collect(Collectors.toList());

        return productDetailsList.stream()
                .map(productDetails -> Product.builder()
                        .fullName(productDetails.getTitle())
                        .description(productDetails.getDescription())
                        .priceList(getJoinedList(priceService.getPriceByProductTitle(productDetails.getTitle()),
                                getPrice(productDetails)))
                        .techSpecs(techCharacteristicsService.getTechCharacteristicsByProductId(productDetails.getId()))
                        .build())
                .collect(Collectors.toList());
    }
